...
const _takeWhile = _curry((func, iter) => {
  let res = [];
  iter = toIter(iter);
  return (function recur() {
    let cur;
...
const _takeUntil = _curry((func, iter) => {
  let res = [];
  iter = toIter(iter);
  return (function recur() {
    let cur;
    while (!(cur = iter.next()).done) {
      const a = cur.value;
      const b = _go1(a, a => (res.push(a), func(a, res)));
      if (b instanceof Promise) {
        return b
          .then(b => (b ? res : recur()))
          .catch(e => (e === nop ? recur() : Promise.reject(e)));
...
const _tap = (f, ...fs) => {
  return (a, ...as) => _go1(_reduce(_call, f(a, ...as), fs), _ => a);
};
...
L._map = _curry(function* (func, iter) {
  ...
  */
  for (const a of toIter(iter)) yield _go1(a, func);
});
...
L._takeWhile = _curry(function* (f, iter) {
  let prev = resolved,
    ok = true;
  for (const a of toIter(iter)) {
    const _ok = ok && _go1(a, f);
...
L._takeUntil = _curry(function* (f, iter) {
  let prev = null,
    ok = false;
  for (const a of toIter(iter)) {
    const _ok = ok || _go1(a, f);
...
const _each = _curry((f, iter) => {
  return _map(a => _go1(f(a), _ => a), iter);
});
